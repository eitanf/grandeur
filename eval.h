...
Scores computeScores(
        const evaluator_t eval,                  // Evaluation function for a given board
        const Moves& moves,                      // List of moves we wish to score
        const player_id_t pid,                   // Player making move
        const Board& curBoard,                   // The current board state
        const Cards& hidden,                     // The hidden cards held by the player
        std::vector<Board>* newBoards = nullptr, // Output: the boards generated by each move
        std::vector<Moves>* newMoves = nullptr   // Output: the legal move lists generated by each move
);
